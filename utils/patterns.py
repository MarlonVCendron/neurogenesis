...
def generate_similar_pattern(pattern, overlap_p):
  N = len(pattern)
...
def generate_partial_pattern(pattern, overlap_p):
  """Return a cue with only overlap_p fraction of the original active neurons (no noise added)."""
  one_indices = np.where(pattern == 1)[0]
  keep_num = round(overlap_p * len(one_indices))
  keep_ones = np.random.choice(one_indices, keep_num, replace=False)
  new_pattern = np.zeros(len(pattern), dtype=int)
  new_pattern[keep_ones] = 1
...
def generate_similar_patterns(pattern, step):
  ...
  for overlap_p in np.arange(step, 1.0, step):
    new_pattern = generate_similar_pattern(pattern, overlap_p)
