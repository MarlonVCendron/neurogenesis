...
\end{tabular}}
\caption{%
  \textit{In vivo} firing rates and population sparsity for each cell type, alongside model values (mean). \textit{In vivo} values
  are mean $\pm$ SD\@. \textsuperscript{max} maximum observed rate. $\approx$ approximate value read from figure. Model firing
  rates are the mean over active cells; values are from the control network, except the immature granule cells (absent in the
  control), which are from the neurogenesis model at 50\% EC$\to$iGC excitability. }
