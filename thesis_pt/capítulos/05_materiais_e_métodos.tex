...
Baseada principalmente nos modelos de~\citeonline{kopsickFormation2024,kimAdult2024,yangDynamic2025,chavlisDendrites2017}, a arquitetura
...
$x_{EC \to iGC}$ da probabilidade de conexão entre o EC e as mGCs. O Hippocampome.org~\cite{wheelerHippocampomeorg2023} não
fornece as características das iGCs. Portanto, os parâmetros utilizados para as iGCs foram ajustados a partir dos dados de
disparos de~\citeonline{espositoNeuronal2005} utilizando o método de~\citeonline{nelderSimplex1965}
...
seguindo a formulação de~\citeonline{sennAlgorithm2001}\cite{mongilloSynaptic2008a}. Esse modelo modela a plasticidade de curto prazo,
...
de plasticidade será baseado no trabalho de~\citeonline{kopsickFormation2024}, que utiliza uma regra de plasticidade dependente do
...
Após a fase inicial de aprendizagem, as iGCs passarão por um processo de maturação simulada discreta, transformando-se em mGCs de
...
Um valor de $\mathcal{S}_D > 1$ indica que os padrões de saída são mais distintos que os de entrada, caracterizando a separação de padrões. Inversamente, $\mathcal{S}_D < 1$ indica uma convergência de padrões, onde os padrões de saída se tornam mais similares entre si.
...
por~\citeonline{kopsickFormation2024}, que implementou um modelo em escala real do CA3 usando também os dados do Hippocampome.org,
