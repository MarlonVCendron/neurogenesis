...
from utils.patterns import orthogonalization_degree
...
def pearson_separation_degree(in_1, in_2, out_1, out_2):
    d_in  = orthogonalization_degree(in_1, in_2)
    d_out = orthogonalization_degree(out_1, out_2)
    if d_in == 0:
        return d_out
    return d_out / d_in
...
def in_similarity():
  ...
  for group in groups:
    ...
    for trial in data[group]:
      ...
      for pattern in trial['patterns'][:-1]:
        ...
        s_d = pearson_separation_degree(original_inp, inp, original_out, out)
        i_s_d = pearson_separation_degree(original_inp, inp, original_iout, iout)
        m_s_d = pearson_separation_degree(original_inp, inp, original_mout, mout)
  ...
  plt.ylabel('Othorgonalization degree ($\\mathcal{O}_D$)')
  ...
  # plt.show()
  plt.tight_layout()
  plt.savefig(f'figures/plots/avg_othorgonalization.jpg', dpi=300, format='jpg', bbox_inches='tight')
  plt.savefig(f'figures/plots/avg_othorgonalization.pdf', format='pdf', bbox_inches='tight')
  plt.close()
