    ...
    for trial in data[group]:
        ...
        for pattern in trial['patterns'][:-1]:
            ...
            ca3_sds.setdefault(sim, []).append(
                pattern_separation_degree(orig_pp, pp, orig_ca3,  pattern['pca3_pattern']))
            mgc_sds.setdefault(sim, []).append(
                pattern_separation_degree(orig_pp, pp, orig_mgc,  pattern['mgc_pattern']))
            igc_sds.setdefault(sim, []).append(
                pattern_separation_degree(orig_pp, pp, orig_igc,  pattern['igc_pattern']))

    mean = lambda d: np.mean([np.mean(v) for v in d.values()])
    se   = lambda d: sem([np.mean(v) for v in d.values()])
    return (mean(ca3_sds), mean(mgc_sds), mean(igc_sds),
            se(ca3_sds),   se(mgc_sds),   se(igc_sds))
